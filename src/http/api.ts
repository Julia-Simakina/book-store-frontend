...
export const createUser = async (
  ...
  try {
    const response: AxiosResponse<AuthUserType> = await api.post(
      "api/auth/signup",
      userData
    );
...
export const loginUser = async (
  ...
  try {
    const response: AxiosResponse<ResponseLoginType> = await api.post(
      "api/auth/signin",
      userData
    );
    return response.data;
...
export const updateUser = async (userId: number): Promise<UserType> => {
  try {
    ...
    console.error("Error fetching update user data by ID: ", error);
    throw error;
  }
};
